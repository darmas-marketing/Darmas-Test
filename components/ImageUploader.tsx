...
const ImageUploader: React.FC<ImageUploaderProps> = ({ onImageUpload }) => {
  ...
  const handleFileChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && ['image/jpeg', 'image/png', 'image/webp'].includes(file.type)) {
      ...
      reader.readAsDataURL(file);
        ...
        setFileName(null);
        alert("Bitte wählen Sie eine gültige Bilddatei (JPEG, PNG, WEBP).");
    }
  }, [onImageUpload]);
  ...
  return (
    <div className="w-full bg-white p-6 rounded-xl border border-gray-200">
      <h3 className="text-lg font-semibold text-gray-800 mb-4">1. Ihr Werbebild hochladen</h3>
      <label
      ...
      >
        {imagePreview ? (
          <div className="relative w-full h-full">
            <img src={imagePreview} alt="Vorschau" className="w-full h-full object-contain rounded-lg p-2" />
          ...
          <div className="flex flex-col items-center justify-center pt-5 pb-6 text-center">
            ...
            <p className="mb-2 text-sm text-gray-500"><span className="font-semibold">Zum Hochladen klicken</span> oder per Drag & Drop</p>
